...
public class IFrameworkDriverManager {
    private static ThreadLocal<IFrameworkWebDriver> _iframeworkWebDriver =
            new ThreadLocal<IFrameworkWebDriver>();
    ...
    public static void closeWebDriver() {
        ...
        if (webDriver != null) {
            System.out.println(
                    " === CLOSE WEBDRIVER: " + Thread.currentThread().getId() + " -> " + webDriver);

            try {
                RemoteWebDriver remoteWebDriver = (RemoteWebDriver) webDriver;
                if (remoteWebDriver.getSessionId() != null) {
                    boolean isSafariOniOS =
                            getIFrameworkDriver()
                                    .getDeviceName()
                                    .equalsIgnoreCase(IFrameworkConstants.DEVICE_IOS)
                                    && getIFrameworkDriver()
                                    .getApplicationType()
                                    .equalsIgnoreCase(IFrameworkConstants.APP_TYPE_SAFARI);
                    if (isSafariOniOS) {
                        webDriver.close();
                    }
                ...
                System.err.println("Cannot close browser normally: " + cannotCloseBrowser.getMessage());
            } finally {
